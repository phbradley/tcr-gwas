...
plot_pca = function(data_set_name) {
    ...
    for (genotype in unique(cleaned$genotype_str)) {
        ...
        full_data_set_name = paste(data_set_name, genotype)
        par(mfrow=c(1,2))
        biplot(princomp(acomp(freqs)), main=paste("compositional: ", full_data_set_name), expand=0.8)
        biplot(princomp(freqs), main=paste("naive: ", full_data_set_name), expand=0.8)
    }
}
