...
sig_artemis = fread(paste0(OUTPUT_PATH, '/source_data/figure4-source-data1.txt'))

sig_artemis[productive == TRUE, productivity := 'productive']
sig_artemis[productive == FALSE, productivity := 'non-productive']

feature_colors = brewer.pal(n = max(4, length(unique(sig_artemis$phenotype))), name = "Set2")

plot = ggplot(sig_artemis, aes(y = effect_size, x = productivity)) +
...
ggsave(paste0(PROJECT_PATH, '/tcr-gwas/figures/sig_artemis_effect_size_boxplot.pdf'), plot = plot, width = 14, height = 10, units = 'in', dpi = 750, device = cairo_pdf)
