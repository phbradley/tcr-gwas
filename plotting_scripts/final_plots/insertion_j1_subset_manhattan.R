...
features = c('vd_insert_j1_subset', 'dj_insert_j1_subset')
pretty_names = c('V-D-gene junction N-insertions', 'D-J-gene junction N-insertions')

manhattan_plot(dataframe = compile_manhattan_plot_data(features), 
               phenotype_values = pretty_names,     
               plot_title = '', 
               file_name = make_file_name('insertion_j1_subset'), 
               plotting_p_value_cutoff = plotting_cutoff)
assign('insertion_manhattan', readRDS(paste0(make_file_name('insertion_j1_subset'), '.rds')))
...
file_name = paste0(make_file_name('insertion_j1_subset'), '.pdf')
ggsave(file_name, plot = revised_insertion + background_grid(major = 'y'), width = 25, height = 18, units= 'in', dpi = 750, device = cairo_pdf)
