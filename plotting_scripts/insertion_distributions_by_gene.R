...
plot_insertions_distribution_by_gene <- function(gene_type, insertion_type, by_allele, frequency_filter){
    ...
    for (gene in gene_type){
        for (insert in insertion_type){
            ...
            ggplot(file, aes_string(x = paste(insert), color = paste(gene_type_plot))) + facet_wrap(~productivity_status, ncol = 1, nrow = 2) + geom_density(aes(y = ..scaled..), size = 0.5, adjust = 4) + ggtitle(paste0(insert, ' distribution by ', gene)) + theme_bw() + theme(title=element_text(size=20, hjust=0.5), axis.title=element_text(size=18), axis.text = element_text(size=18), legend.text = element_text(size=16), strip.text = element_text(face="bold", size=18)) 

            ggsave(paste0('insertion_distribution_', insert, '_by_', gene, '.png'), plot = last_plot(), height=8, width=16, units="in") 
        }
    }
}
