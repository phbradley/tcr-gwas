...
run_snps_trimming_snp_list(snp_id_list = unique(snp_data$snp), trim_type = args[1], gene_type = args[2], condensing = 'by_gene', gene_conditioning = 'True', weighting = 'True', repetitions = 10, write_table = 'True')
print(paste0("Finished regressions for ", args[1], " for ", args[3])
#run_snps_trimming_snp_list(snp_id_list = unique(snp_list$snpid), trim_type = args[1], condensing = 'by_patient', gene_conditioning = 'False', weighting = 'False', repetitions = 100)v
