...
correlate_snps <- function(snp_meta_data, productivity, trim_type, bonferroni, proximity_cutoff, pvalue_cutoff){
    ...
    while (j < nrow(together)){
        if (together[j]$pvalue < bonferroni){
            ...
            index = j
            for (i in seq(j-1, 1)){
                if ((abs(together[i]$hg19_pos - together[index]$hg19_pos )<= proximity_cutoff) && (together[i]$pvalue < pvalue_cutoff)){
    ...
    colnames(correlated_snps) = c("index","snp", "standard_error", "intercept", "slope", "zscore", "p", "chr", "start", "mwu_P")
    correlated_snps$end = correlated_snps$start
    return(correlated_snps)
}


group_p_values <- function(snp_meta_data, productivity, trim_type, bonferroni, proximity_cutoff, pvalue_cutoff){
    correlated_snps = correlate_snps(snp_meta_data, productivity, trim_type, bonferroni, proximity_cutoff, pvalue_cutoff)
