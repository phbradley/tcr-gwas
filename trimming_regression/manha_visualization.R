...
compile_all_data_manhattan_from_cluster <- function(trim_type, random_effects, bootstrap_count){
    bonferroni = 0.05/35481497
    ...
    data_files = list.files(path=paste0('/fh/fast/matsen_e/shared/tcr-gwas/trimming_regression_output/cluster_job_results/',trim_type), pattern=file_pattern, full.names=TRUE)

    par(mar=c(5,5,7,5)+.1)
    xlimit = c(0, 3200000000)
    ...
    for (file in data_files){
        # read file...
        temp_file = as.data.frame(read.table(file, sep = "\t", fill=TRUE, header = TRUE))
        together = rbind(together, temp_file)
    }
    together = together[order(together$chr, together$hg19_pos),]
    together_productive = together %>% filter(productivity == 'productive')
    together_NOT_productive = together %>% filter(productivity == 'NOT_productive')


    #colors = with(prod_status, data.frame(continent = unique(prod_status), color = I(brewer.pal(3, name = 'Set3')))))   

    #palette(brewer.pal(n = length(unique(prod_status)), name = 'Set3'))
    #col = setNames(palette(), levels(as.factor(prod_status)))

    plotProductive = manhattanPlot(together_productive$pvalue, together_productive$chr, ylim = c(0,200), trunc.lines = FALSE, signif = bonferroni, thinThreshold=NULL, pointsPerBin=10000)
    plotNOTProductive = manhattanPlot(together_NOT_productive$pvalue, together_NOT_productive$chr, ylim = c(0,200), trunc.lines = FALSE, signif = bonferroni, thinThreshold=NULL, pointsPerBin=10000)
    }