...
run_snps_trimming_snp_list_cluster_by_race <- function(snp_list, genotype_list, trim_type, pca_structure_correction, pca_type, write_table){
    ...
    together = merge(trimming_data, ethnicity_data[,-c('scanID')], by = 'localID')  
    ...
    count = 0
    for (race_index in unique(together$race.g)){
        results = NULL
        if (nrow(together[race.g == race_index])>9){
            ...
            results = foreach(snp=list_of_snps, .combine='rbind') %dopar% {
                ...
                execute_regression(snp, 
                                   list_of_snps, 
                                   snp_list, 
                                   genotype_list[row.names(genotype_list) %in% together[race.g == race_index]$localID,], 
                                   trim_type, 
                                   together[race.g == race_index], 
