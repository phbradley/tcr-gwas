...
execute_regression <- function(snp, list_of_snps, snp_list, genotype_list, trim_type, condensing, trimming_data, repetitions, pca_structure_correction, weighting, gene_conditioning, random_effects, pvalue_boot_threshold, regression_dataframe){
    if (as.character(snp) %in% colnames(genotype_list)){
        ...
        if (length(unique(snp_genotypes$snp)[!is.na(unique(snp_genotypes$snp))]) <= 1){
            ...
            regression_productive = suppressMessages(trimming_regression(snps_dataframe = snp_genotypes, condensed_trimming_dataframe = trimming_data, condensing, productive = "True", trim_type = trim_type, bootstrap_repetitions = repetitions, pca_structure_correction, gene_conditioning, weighting, random_effects, pvalue_boot_threshold, snp_list))
            regression_NOT_productive = suppressMessages(trimming_regression(snps_dataframe = snp_genotypes, condensed_trimming_dataframe = trimming_data,condensing, productive = "False", trim_type = trim_type, bootstrap_repetitions = repetitions,pca_structure_correction, gene_conditioning, weighting, random_effects, pvalue_boot_threshold, snp_list))
