...
run_snps_trimming_snp_list_cluster <- function(snp_list, genotype_list, trim_type, gene_type, condensing, gene_conditioning, weighting, random_effects, repetitions, write_table, ncpus, d_infer, maf_cutoff){
    ...
    results = foreach(snp=list_of_snps, .combine='rbind') %dopar% {
        ...
        execute_regression(snp, list_of_snps, snp_list, genotype_list, trim_type, gene_type, condensing, trimming_data, repetitions, weighting, gene_conditioning, random_effects, regression_dataframe)
        }
    stopImplicitCluster()
    
    if (random_effects == 'True'){
        if (d_infer == 'False') {
            file_name = paste0('/fh/fast/matsen_e/shared/tcr-gwas/trimming_regression_output/cluster_job_results/NO_d_infer/',trim_type, '/', trim_type, '_',snp_list$snp[1], '-', snp_list$snp[nrow(snp_list)],'_snps_regression_with_weighting_condensing_by_gene_with_random_effects_', repetitions, '_bootstraps_NO_d_infer.tsv')
        } else {
            file_name = paste0('/fh/fast/matsen_e/shared/tcr-gwas/trimming_regression_output/cluster_job_results/',trim_type, '/', repetitions, '_bootstraps/', trim_type, '_',snp_list$snp[1], '-', snp_list$snp[nrow(snp_list)],'_snps_regression_with_weighting_condensing_by_gene_with_random_effects_', repetitions, '_bootstraps.tsv')
        }
    } else {
        if (d_infer == 'False') {
            file_name = paste0('/fh/fast/matsen_e/shared/tcr-gwas/trimming_regression_output/cluster_job_results/NO_d_infer/',trim_type, '/', trim_type, '_',snp_list$snp[1], '-', snp_list$snp[nrow(snp_list)],'_snps_regression_with_weighting_condensing_by_gene_NO_random_effects_', repetitions, '_bootstraps_NO_d_infer.tsv')
        } else {
            file_name = paste0('/fh/fast/matsen_e/shared/tcr-gwas/trimming_regression_output/cluster_job_results/',trim_type, '/', repetitions, '_bootstraps/', trim_type, '_',snp_list$snp[1], '-', snp_list$snp[nrow(snp_list)],'_snps_regression_with_weighting_condensing_by_gene_NO_random_effects_', repetitions, '_bootstraps.tsv')
        }
    }
