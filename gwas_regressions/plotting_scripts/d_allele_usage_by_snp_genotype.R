...
filename = set_file_name(top_snp, type = 'allele_usage', final_figure = TRUE)

final_plot = plot +  theme_cowplot(font_family = 'Arial') + theme(legend.position = 'none', axis.text = element_text(size = 24), axis.line = element_blank(),text = element_text(size = 40), axis.ticks = element_line(color = 'gray60', size = 1.5)) + ggtitle('') + background_grid(major = 'y') 

ggsave(filename, plot = final_plot, width = 14, height = 10, units = 'in', dpi = 750, device = cairo_pdf)
