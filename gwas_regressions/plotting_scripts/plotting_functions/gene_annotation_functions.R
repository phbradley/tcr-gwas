...
get_gene_features <- function(gene_name, features){
    ...
    for (feature in features){
        ...
        temp_feature_dt = fread(paste0(PROJECT_PATH, '/tcr-gwas/_ignore/gene_features/', gene_name, '_', feature, '.txt'))[, 1:3]
...
get_genes_in_tcrb <- function(simplify){
    tcrb_features = fread(paste0(PROJECT_PATH, '/tcr-gwas/_ignore/gene_features/tcrb_features_gencode_24_37.txt'), fill = TRUE)[, c('chrom', 'txStart', 'txEnd', 'name2')]
