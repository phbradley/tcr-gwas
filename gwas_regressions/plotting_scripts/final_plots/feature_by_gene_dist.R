...
plot_distributions_by_gene <- function(data, gene_types, paired_feature_types, plot_variable_names, pretty_gene_names){
    ...
    final_plot = plot + theme_cowplot() + theme(legend.position = "none", axis.text = element_text(size = 24), axis.line = element_blank(), text = element_text(size = 40), axis.ticks = element_line(color = 'gray60', size = 1.5)) + coord_cartesian(clip="off") + ggtitle('') + background_grid(major = 'xy') + panel_border(color = 'gray60', size = 1.5)

    filename = create_filename(paired_feature_types)
    ggsave(paste0(PROJECT_PATH, '/tcr-gwas/gwas_regressions/figures/', filename), plot = final_plot, width = 15, height = 20, units = 'in', dpi = 750, device = 'pdf')
}

plot_distributions_by_gene(insertions, c('v_gene', 'd_gene', 'd_gene', 'j_gene'), c('vd_insert', 'vd_insert', 'dj_insert', 'dj_insert'), c('V-D-gene insertions', 'D-J-gene insertions'), c('V-gene','D-gene', 'J-gene'))
plot_distributions_by_gene(trimmings, c('v_gene', 'd_gene', 'd_gene', 'j_gene'), c('v_trim', 'd0_trim', 'd1_trim', 'j_trim'), c('V-gene trimming', '5\'-D-gene trimming', '3\'-end-D-gene trimming', 'J-gene trimming'), c('V-gene', 'D-gene', 'J-gene'))
