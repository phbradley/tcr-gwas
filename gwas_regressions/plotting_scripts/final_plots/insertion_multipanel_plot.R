...
revised_insertion = insertion_manhattan + theme_cowplot() + theme(legend.direction = "horizontal",legend.position="bottom", legend.justification = 'left', panel.spacing.y=unit(3, "lines"), axis.ticks.x = element_blank(), panel.spacing.x=unit(0, "lines"), strip.placement = 'outside', strip.background.x = element_blank(), text = element_text(size = 40), axis.text.x = element_blank(), strip.text.x = element_text(size = 12), axis.text.y = element_text(size = 24), axis.line.y = element_line(color = 'gray60', size = 1.5), axis.line.x = element_blank(), axis.ticks = element_line(color = 'gray60', size = 1.5)) + coord_cartesian(clip="off") + geom_hline(aes(yintercept = Inf), size = 1.5, color = 'gray60')+geom_hline(aes(yintercept = -Inf), size = 1.5, color = 'gray60') + scale_x_continuous() + geom_point(aes(x = Inf, y = -Inf), size = 3, color = 'gray60', shape = '\u007c') + ggtitle('') + ylim(2, 12.5) + background_grid(major = 'y')


revised_dntt = insertion_manhattan_dntt + theme_cowplot() + theme(legend.position = "none", panel.spacing.y=unit(3, "lines"), panel.spacing.x=unit(0, "lines"), strip.placement = 'outside', strip.background.x = element_blank(), text = element_text(size = 40), strip.text.x = element_blank(), axis.text.x = element_blank(), axis.text.y = element_text(size = 24), axis.line.y = element_line(color = 'gray60', size = 1.5), axis.line.x = element_blank(), axis.ticks.x = element_blank(), axis.ticks.y = element_line(color = 'gray60', size = 1.5)) + coord_cartesian(clip="off") + geom_hline(aes(yintercept = Inf), size = 1.5, color = 'gray60')+geom_hline(aes(yintercept = -Inf), size = 1.5, color = 'gray60') + scale_x_continuous()  + ggtitle('') + xlab('Chr 10 Position') + ylim(2, 12.5) + background_grid(major = 'y')
   
aligned_plots = align_plots(revised_insertion, revised_dntt, align = 'h', axis = 'bt')

together = plot_grid(aligned_plots[[1]], aligned_plots[[2]], ncol = 2, rel_widths = c(2, 0.8), label_size = 40, labels = c('A', 'B'), scale = 0.97)
