...
condense_individual_tcr_repertoire_data <- function(tcr_repertoire_dataframe){
    ...
    tcr_repertoire_data_pnuc_tcr_counts = tcr_repertoire_data[get(parameter) >0, .N, by= .(localID, productive, cdr3_gene_group)]
    setnames(tcr_repertoire_data_pnuc_tcr_counts, 'N', paste0(parameter, '_count'))
    # merge 
    together = merge(tcr_repertoire_data_weights, tcr_repertoire_data_pnuc_tcr_counts, all.x = TRUE)
...
condense_individual_tcr_repertoire_data <- function(tcr_repertoire_dataframe){
    ...
    parameter = gsub('_fraction', '', PHENOTYPE)
    tcr_repertoire_data[[paste0(parameter, '_identity')]] = ifelse(tcr_repertoire_data[[parameter]] > 0, TRUE, FALSE)
    tcr_repertoire_data$tcr_count = nrow(tcr_repertoire_data)
    names = c('localID', paste(CONDITIONING_VARIABLE), 'productive')
    fraction_names = c('localID', paste(CONDITIONING_VARIABLE), 'productive', paste0(parameter, '_identity'))
    
    condensed_tcr_repertoire_data = tcr_repertoire_data[, paste0(GENE_TYPE, '_count') := .N, by = mget(names)][, paste0(PHENOTYPE) := .N/get(paste0(GENE_TYPE, '_count')), by = mget(fraction_names)]
    
    condensed_tcr_repertoire_data = condensed_tcr_repertoire_data[get(paste0(parameter, '_identity')) != FALSE]

    condensed_tcr_repertoire_data = condensed_tcr_repertoire_data[, lapply(.SD, mean), by = mget(names), .SDcols = sapply(tcr_repertoire_data, is.numeric)]
