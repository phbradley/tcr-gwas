# CDR3 sequence to gene file
CDR3_GENE_ASSIGNMENT_FILE = paste0(PROJECT_PATH, '/tcr-gwas/_ignore/human_vj_alleles_alpha.tsv')

# Nicaraguan data path
TCR_REPERTOIRE_DATA_DIRECTORY = paste0(PROJECT_PATH, '/tcr-gwas/_ignore/nicaraguan_stats/TCRA/')

VALIDATION_SNP_PATH = paste0(PROJECT_PATH, '/tcr-gwas/_ignore/nicaraguan_snp_data/snp_genotypes_ints.tsv')

PCA_FILE = paste0(PROJECT_PATH, '/tcr-gwas/_ignore/nicaraguan_snp_data/snp_ancestry_PCA.tsv')
