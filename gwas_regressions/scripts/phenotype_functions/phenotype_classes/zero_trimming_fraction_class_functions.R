...
condense_individual_tcr_repertoire_data <- function(tcr_repertoire_dataframe){
    ...
    parameter = gsub('_zero_trimming_fraction', '', PHENOTYPE)
    ...
    tcr_repertoire_data_no_trim_tcr_counts = tcr_repertoire_data[get(parameter) == 0, .N, by= .(localID, productive, cdr3_gene_group)]
    setnames(tcr_repertoire_data_no_trim_tcr_counts, 'N', paste0(parameter, '_zero_trim_count'))
    # merge 
    together = merge(tcr_repertoire_data_weights, tcr_repertoire_data_no_trim_tcr_counts, all.x = TRUE)
    ...
    if (length(together[is.na(together[[paste0(parameter, '_count')]])][[paste0(parameter, '_count')]]) > 0){
        together[is.na(together[[paste0(parameter, '_zero_trim_count')]])][[paste0(parameter, '_zero_trim_count')]] <- 0
    }
    # define weights
    together[[paste0('weighted_', GENE_TYPE, '_count')]] = together[[paste0(GENE_TYPE, '_count')]]/tcr_total
    #define fraction phenotype
    together[[PHENOTYPE]] = together[[paste0(parameter, '_zero_trim_count')]]/together[[paste0(GENE_TYPE, '_count')]]
