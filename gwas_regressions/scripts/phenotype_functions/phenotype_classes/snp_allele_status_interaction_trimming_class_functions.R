...
set_regression_formula <- function(snp){
    ...
    # formula_string = paste0(PHENOTYPE, ' ~ `', snp, '` + as.factor(', CONDITIONING_VARIABLE, ') + as.factor(TRBD2_alt_allele_genotype) +')
    formula_string = paste0(PHENOTYPE, ' ~ `', snp, '` + as.factor(', CONDITIONING_VARIABLE, ') + `', snp, '`:as.factor(TRBD2_alt_allele_genotype) +')
