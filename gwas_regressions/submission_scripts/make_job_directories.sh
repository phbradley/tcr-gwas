...
for JOB in {01..35481497..1000}; do
    mkdir -p $PROJECT_PATH/tcr-gwas/gwas_regressions/current_cluster_job_directories/${PHENOTYPE}/${PHENOTYPE}_${JOB}/config
    ...
    set -eu
    Rscript $PROJECT_PATH/tcr-gwas/gwas_regressions/scripts/execute_regressions.R ${JOB} $PHENOTYPE $(echo '$1') 
    ...
    cp $PROJECT_PATH/tcr-gwas/gwas_regressions/config/config.R $PROJECT_PATH/tcr-gwas/gwas_regressions/current_cluster_job_directories/${PHENOTYPE}/${PHENOTYPE}_${JOB}/config 
